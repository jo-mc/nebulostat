...
//QuantRoller calculates a rolling quantile
...
func RollingStat(x float64, rS *RStats) {
	...
	rS.N = rS.N + 1
	delta = x - rS.M1
	deltaN = delta / float64(rS.N)
	deltaN2 = deltaN * deltaN
	term1 = delta * deltaN * n1
	n1 = n1 + 1
	rS.M1 = rS.M1 + deltaN
	rS.M4 = rS.M4 + term1*deltaN2*(n1*n1-3*n1+3) + 6*deltaN2*rS.M2 - 4*deltaN*rS.M3
	rS.M3 = rS.M3 + term1*deltaN*(n1-2) - 3*deltaN*rS.M2
	rS.M2 = rS.M2 + term1
...
//adelta = x - mean
//deltaN= adelta / n
//delta_nsq = deltaN* delta_n
//term1 = adelta * deltaN* (n - 1)
//mean = mean + delta_n
//M4 = M4 + term1 * delta_nsq * (n * n - 3 * n + 3) + 6 * delta_nsq * M2 - 4 * deltaN* M3
